...
printf 'knowledge: Watari\nplanning: Sherlock\nhealth: Guy Roux\n' > answers1.yml
python3 install.py --answers answers1.yml >/dev/null 2>&1
[ -f .claude/agents/watari.md ] && ok "renders an advisor under its slugified handle" \
  || no "renders advisor" "watari.md missing"
[ "$(sed -n '2p' .claude/agents/guy-roux.md)" = "name: guy-roux" ] \
  && ok "frontmatter name is the slug of the display name" \
  || no "slug in frontmatter" "$(sed -n '2p' .claude/agents/guy-roux.md)"
[ -f .claude/skills/watari-intake/SKILL.md ] && ok "each advisor gets its own intake skill" \
...
printf 'knowledge: Alfred\nplanning: Sherlock\nhealth: Guy Roux\n' > answers2.yml
out="$(python3 install.py --answers answers2.yml 2>&1)"

[ ! -f .claude/agents/watari.md ] && ok "renaming an advisor retires the old agent file" \
  || no "old agent retired" "watari.md survived"
[ ! -d .claude/skills/watari-intake ] && ok "renaming retires the old intake skill" \
  || no "old intake retired" "watari-intake survived"
[ -f .claude/agents/alfred.md ] && ok "the renamed advisor is installed" || no "new advisor" "missing"
...
[ -d "Team's brain/watari" ] && ok "working memory is preserved across a rename" \
  || no "brain preserved" "deleted"
printf '%s' "$out" | grep -q "Team's brain/watari/" && ok "orphaned brain folders are reported" \
  || no "orphan reported" "not mentioned"
[ -f .claude/agents/sherlock.md ] && ok "an unchanged advisor survives a re-run" \
